...
async function ouvrirAchatDetail() {
    ...
    // 🔥 Vérifier les commandes gratuites
    const commandesGratuites = await verifierCommandesGratuites();
    console.log('🎁 Commandes sans frais restantes:', commandesGratuites); // Debug
...
window.installPWA = async function() {
  ...
  if (outcome === 'accepted') {
    ...
    closeInstallBanner(); // Fermer après installation
  } else {
...
window.addEventListener('appinstalled', (evt) => {
  console.log('✅ jëndUp a été installé sur l\'appareil');
  // Cacher la bannière définitivement
  localStorage.setItem('installBannerClosed', 'true');
  document.getElementById('install-banner').style.display = 'none';
});

// Cacher la bannière par défaut au chargement
